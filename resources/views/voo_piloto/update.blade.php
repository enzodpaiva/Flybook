...
<div class="container">
    ...
    <form class="form-horizontal" method="POST" action="/piloto/voo-update/{{$voo->id}}">
        @csrf
<fieldset>
...
<div class="form-group">
  ...
  <div class="col-md-8">
  <input id="direcao" value="{{$voo->direcao_vento}}" name="direcao" placeholder="" class="form-control input-md" required="" type="text">
  </div>
</div>
...
<div class="form-group">
  ...
  <div class="col-md-8">
  <input id="visibilidade" value="{{$voo->visibilidade}}" name="visibilidade" placeholder="" class="form-control input-md mb-4" required="" type="text">
  </div>
</div>
...
<input type="time" value="{{$voo->inicio}}" id="inicio" name="inicio"
       min="00:00" max="24:00" required>
       <label for="appt">Hora do Fim:</label>

<input type="time" value="{{$voo->fim}}" id="fim" name="fim"
       min="00:00" max="24:00" required>
</div>
...
<div class="form-group">
  ...
  <div class="col-md-8">
  <input id="distancia" value="{{$voo->distancia}}" name="distancia" placeholder="Apenas números" class="form-control input-md mb-4" required="" type="number" maxlength="11" pattern="[0-9]+$">
  </div>
  ...
  <div class="col-md-8">
  <input id="altura" value="{{$voo->altura}}" name="altura" placeholder="Apenas números" class="form-control input-md mb-4" required="" type="number" maxlength="11" pattern="[0-9]+$">
  </div>

  <div class="form-group mb-4">
  <label class="col-md-2 control-label" for="Nome">Data<h11>*</h11></label>  
  <div class="col-md-8">
  <input id="data" value="{{$voo->data}}" name="data" placeholder="DD/MM/AAAA" class="form-control input-md mb-4" required="" type="text" maxlength="10" OnKeyPress="formatar('##/##/####', this)" onBlur="showhide()">
</div>

<div class="form-group">
  <label class="col-md-6 control-label" for="Nome">Velocidade do Vento (km/h)<h11>*</h11></label>  
  <div class="col-md-8">
  <input id="velocidade" value="{{$voo->velocidade_vento}}" name="velocidade" placeholder="Apenas números" class="form-control input-md mb-4" required="" type="number" maxlength="11" pattern="[0-9]+$">
  </div>
...
{{-- <label class="col-md-4 control-label" for="radios">Termais <h11>*</h11></label>
  ...
  <div class="mb-4">
        ...
        <option value="{{$cat->id}}"
            @if ($cat->id==$voo->instrutor_id)
                 selected="selected"
                 @endif>{{$cat->name}} - {{$cat->cpf}}</option>
              @endforeach
            {{-- <option value=""></option>
      ...
      <em>Rampa<h11>*</h11></em>
        <select data-placeholder="Your Favorite Type of Bear" name="rampa"  class="chosen-select" tabindex="7">
            ...
            <option value="{{$cat->id}}"
                @if ($cat->id==$voo->rampa_id)
                 selected="selected"
                 @endif
                 >{{$cat->local}} - altitude: {{$cat->altitude}}m</option>
            @endforeach
            {{-- <option value=""></option>
          <option>American Black Bear</option>
          <option>Asiatic Black Bear</option>
      ...
      <em>Asa<h11>*</h11></em>
        <select data-placeholder="Your Favorite Type of Bear" name="asa" class="chosen-select" tabindex="7">
            ...
            @foreach(App\AsaDelta::all() as $cat)
            <option value="{{$cat->id}}"
                @if ($cat->id==$voo->asaDelta_id)
                 selected="selected"
                 @endif
                 >{{$cat->nome}} - ultima manutenção: {{$cat->validade_manutencao}}</option>
            @endforeach
            {{-- <option value=""></option>
          <option>American Black Bear</option>
          <option>Asiatic Black Bear</option>
      ...
      <div class="form-group">
        <em>Observações</em>
      <textarea class="form-control"  name="observacoes" id="exampleFormControlTextarea1" rows="3">{{$voo->observacoes}}</textarea>
      </div>
